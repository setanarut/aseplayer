...
func (g *Game) Draw(s *ebiten.Image) {
	...
	g.dio.GeoM.Reset()
	g.dio.GeoM.Translate(g.fly.CurrentFrame.Position.X, g.fly.CurrentFrame.Position.Y)
	...
	g.dio.GeoM.Reset()
	g.dio.GeoM.Translate(g.subfly.CurrentFrame.Position.X, g.subfly.CurrentFrame.Position.Y)
...
func ParseUserDataPivots(ap *aseplayer.AnimPlayer) {
	for _, anim := range ap.Animations {
		for i := range anim.Frames {
			off := parseOffset(anim.UserData)
			anim.Frames[i].Position = anim.Frames[i].Position.Sub(off)
		}
	}
}
